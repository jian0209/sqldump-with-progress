...
Help()
{
    ...
    echo "Usage: $0 -H=\"localhost\" -P=\"3306\" --output=\"/tmp/output.sql.gz\" --databases=\"database1 database2\" --no-data <user> (<password> | < password.txt)"
    echo ""
    echo "options:"
    ...
    echo "--output      Output file path, will using gzip to store the file with .gz extension, recommended extension: '.sql'.[Default: Output to STDOUT]"
...
(
    ...
    if [ "$databases" != "" ]; then
        ...
        execute_get_size_command=$(mysql -h$host -uroot -p$password -e "SELECT ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) AS 'Size (MB)' FROM information_schema.TABLES WHERE table_schema IN ($databases) GROUP BY table_schema;" | tail -n +2 | grep -v -E '(information_schema|performance_schema|mysql|sys|innodb|tmp)' | awk '{s+=$1} END {print s}')
    fi
    ...
    file_size=$execute_get_size_command

    if [ "$output_file" != "" ]; then
        execute_dump_command="$execute_dump_command | pv -W -s $file_size""M | gzip -c > $output_file"
    else
        execute_dump_command="$execute_dump_command | pv -W -s $file_size""M | gzip -c > /dev/null"
    fi
